...
import { User, Account, UserOTP, JWTPayload, SignupRequest } from '../types/auth';
...
export class AuthService {
  ...
  private generateTokens(user: User, account: Account) {
    ...
    // Ensure secrets are strings and not undefined
  ...
  async refreshToken(refreshToken: string): Promise<{ success: boolean; message: string; access_token?: string }> {
    try {
      // Ensure refresh secret is available
      const jwtRefreshSecret = config.jwt.refreshSecret;
      if (!jwtRefreshSecret) {
        throw new Error('JWT refresh secret is not configured properly');
      }
      ...
      const { accessToken } = this.generateTokens(user, account);
